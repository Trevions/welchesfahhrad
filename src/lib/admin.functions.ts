...
const articleInput = z.object({
  ...
  cover_image_caption: z.string().max(400).optional().nullable(),
  cover_image_credit: z.string().max(300).optional().nullable(),
  cover_image_is_ai: z.boolean().optional().default(false),
...
export const upsertArticle = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const payload: Record<string, any> = {
      ...
      cover_image_credit: data.cover_image_credit || null,
      cover_image_is_ai: !!data.cover_image_is_ai,
