...
export const Route = createFileRoute("/tools/kalorien")({
  head: () =>
    toolHead({
      title: "Kalorien-Rechner Radfahren",
      description:
        "Energieverbrauch beim Radfahren nach MET-Methode (Compendium of Physical Activities) — inkl. Kohlenhydrat- und Wasserempfehlung.",
      path: "/tools/kalorien",
    }),
...
// MET-Werte aus dem Compendium of Physical Activities (2011), Code 01010–01080
...
function KalorienPage() {
  ...
  const result = useMemo(() => {
    ...
    // Höhenmeter-Zuschlag: ~9 kcal / 100 hm / kg (Pendel-Näherung)
    const climbKcal = (climb / 100) * 9 * (weight / 70);
    const kcal = Math.round(base + climbKcal);
    const kj = Math.round(kcal * 4.184);
    // Empfehlungen: 60 g Kohlenhydrate/h ab > 60 min, 500 ml Wasser/h
  ...
  return (
    ...
    >
          ...
          <div className="mt-5 grid gap-4">
            ...
            <div>
              ...
              <ToolInput
                type="number"
                value={climb}
                onChange={(e) => setClimb(+e.target.value || 0)}
              />
            </div>
          </div>
        </ToolCard>
        ...
        <ToolCard>
          ...
          <div className="mt-5 grid gap-4 sm:grid-cols-2">
            ...
            <ToolResultStat
              ...
              value={result.carbs}
              unit="g"
              hint="ab 60 min · 60 g/h"
            />
            <ToolResultStat
              label="Wasser"
              value={result.water}
              unit="ml"
              hint="500 ml/h Faustregel"
            />
          </div>
        </ToolCard>
      </div>
      ...
      <ToolDisclaimer>
        Formel: kcal = MET × kg × Stunden. MET-Werte aus dem Compendium of Physical
        Activities (Ainsworth 2011). Höhenmeter-Zuschlag ≈ 9 kcal pro 100 hm und 70 kg.
        Empfehlungen für Verpflegung folgen ACSM-Standards (60 g KH/h bei Belastungen &gt; 1 h).
      </ToolDisclaimer>
    </ToolShell>
  );
}
